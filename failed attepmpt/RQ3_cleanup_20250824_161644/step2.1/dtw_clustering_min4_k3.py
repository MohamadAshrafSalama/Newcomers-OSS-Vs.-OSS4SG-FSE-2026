...
class ImprovedContributionClustering:
    ...
    def load_and_preprocess(self, target_length=52, min_weeks=6, scaling='per_series'):
        """
        ...
        if scaling == 'minmax':
            ...
            X = scaler.fit_transform(X.T).T
        elif scaling == 'per_series':
            # Scale each series to [0,1] individually to emphasize shape
            X_scaled = []
            for series in X:
                ...
                if series_max > series_min:
                    ...
                    scaled = series * 0  # All same value -> all zeros
...
"""
    ...
    def save_per_cluster_plots(self, X, result, metadata):
        """
        Save individual plots per cluster showing all series (low alpha) and centroid.
        """
        k = result['k']
        labels = result['labels']
        centroids = result['centroids']
        out_dir = self.output_dir / f'per_cluster_k{k}'
        ...
        for cluster_id in range(k):
            ...
            for series in cluster_series:
                plt.plot(series, color=colors[cluster_id], alpha=0.03, linewidth=1)
            # centroid on top
            plt.plot(centroids[cluster_id], color=colors[cluster_id], linewidth=3, alpha=0.95, label=f'Centroid {cluster_id}')
            plt.title(f'Cluster {cluster_id} (n={len(cluster_series)}) - k={k}')
            plt.xlabel('Time Period (weeks)')
            plt.ylabel('Normalized Contribution Index')
            plt.grid(True, alpha=0.3)
            plt.legend()
            fname = out_dir / f'cluster_{cluster_id}_series.png'
            plt.savefig(fname, dpi=150, bbox_inches='tight')
            plt.close()
            self.log(f"Per-cluster plot saved: {fname}", "SAVE")
    ...
    def run_analysis(self, k_values=[2, 3, 4]):
        ...
        X, metadata = self.load_and_preprocess(
            ...
            min_weeks=6,       # Increase to 6 to reduce noise
            scaling='per_series'  # Try per-series scaling
        )
        ...
        for k in k_values:
            ...
            self.save_per_cluster_plots(X, result, metadata)
            ...
            # Save cluster membership and silhouette samples for k=3 only
            if k == 3:
                try:
                    labels = result['labels']
                    sample_sil = silhouette_samples(X, labels)
                    df_membership = metadata.copy()
                    df_membership['cluster'] = labels
                    df_membership['silhouette_sample'] = sample_sil
                    out_csv = self.output_dir / 'cluster_membership_k3.csv'
                    df_membership.to_csv(out_csv, index=False)
                    self.log(f"Cluster membership saved: {out_csv}", "SAVE")
                except Exception as e:
                    self.log(f"Failed to save membership: {e}", "WARNING")
...
def main():
    ...
    # Configuration
    data_path = "/Users/mohamadashraf/Desktop/Projects/Newcomers OSS Vs. OSS4SG FSE 2026/RQ3_engagement_patterns/step1/results/rolling_4week/weekly_pivot_for_dtw.csv"
    output_dir = "clustering_results_min6_per_series/"
    ...
    # Run analysis for k=3 and k=4 to evaluate possible split of large clusters
    results, X, metadata = clustering.run_analysis(k_values=[3, 4])
