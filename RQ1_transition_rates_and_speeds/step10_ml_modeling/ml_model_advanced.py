...
OUTPUT_DIR = BASE_DIR / "RQ1_transition_rates_and_speeds/step10_ml_modeling"
...
class CorePredictionPipeline:
    ...
    def analyze_by_project_type(self, features_df: pd.DataFrame) -> Dict:
        """
        Separate analysis for OSS vs OSS4SG
        """
        print("\n" + "=" * 80)
        print("ANALYZING BY PROJECT TYPE")
        print("=" * 80)
        ...
        for ptype in ['OSS', 'OSS4SG']:
            ...
            y = subset['became_core']
            
            # Standardize
            scaler = StandardScaler()
            X_scaled = scaler.fit_transform(X)
            
            # Train best model (Random Forest based on typical performance)
            ...
            type_results[ptype] = results
            
            print(f"  Samples: {results['n_samples']:,} ({results['n_core']:,} core = {results['pct_core']:.1f}%)")
            print(f"  ROC-AUC: {results['roc_auc']:.3f} ± {results['roc_auc_std']:.3f}")
            print(f"  PR-AUC:  {results['pr_auc']:.3f} ± {results['pr_auc_std']:.3f}")
            print(f"  F1:      {results['f1']:.3f} ± {results['f1_std']:.3f}")
        
        # Save comparison
        comparison_df = pd.DataFrame(type_results).T
        comparison_df.to_csv(self.output_dir / "project_type_comparison.csv")
    ...
    def generate_summary_report(self, features_df: pd.DataFrame, 
        ...
        # Model Performance
        report_lines.append("MODEL PERFORMANCE (5-FOLD CV)")
        report_lines.append("-" * 40)
        for model_name, results in cv_results.items():
